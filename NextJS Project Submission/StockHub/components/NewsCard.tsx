...
export default function NewsCard({ news }: { news: News; key: number }) {
	return (
		<div className={"w-88 flex flex-col gap-2 border-4 shadow-inner rounded-xl p-4 border-gray-500"}>
			...
			<div className={"w-full h-1/2 relative group active:scale-95 transition-transform duration-300"}>
				...
				<img src={news.image_url} alt={news.title} className={"w-full h-full object-cover -z-10 outline outline-4"} />
			</div>
			<div className={"w-full h-fit flex flex-col gap-2"}>
				<span className={"text-white text-sm font-light"}>Sentiment Score: {news.sentiment}</span>
			</div>
